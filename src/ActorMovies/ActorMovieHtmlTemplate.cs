...
{
    public static string ViewAllMoviesByActor(Actor actor, List<(ActorMovie, Movie)> amms, ActorMovie actorMovie, int TotalCount, int page, int size)
    {
        ...
        foreach (var (am, movie) in amms)
        {
            ...
            <td>
            <form action=""/actors/movies/remove?mid={actor.Id}&mid={movie.Id}"" method=""POST"" onsubmit=""return confirm('Are you sure you want to remove this Actor?');"">
        ...
        <div class =""pagination"">                
            ...
            <a href=""?aid={actor.Id}&page={page - 1}&size={size}"" onclick=""return {pDisable};"">Previous</a>
    ...
    public static string ViewAllActorsByMovie(Movie movie, List<(ActorMovie, Actor)> amas, int TotalCount, int page, int size)
    {
        ...
        foreach (var (am, actor) in amas)
        {
            ...
            <td>
            <form action=""/actors/movies/remove?amid={am.Id}"" method=""POST"" onsubmit=""return confirm('Are you sure you want to remove this Actor?');"">
                <input type=""submit"" value=""Remove"">
            </form>
            </td>
            </tr>";
        ...
        string html = $@"
        <div class =""add"">
            <a href=""/movies/actors/add"">Add New ActorMovie</a>
        </div>
        <table class=""viewall"">
            <thead>
                <tr>
        ...
        <div class =""pagination"">                
            <a href=""?aid{movie.Id}&page=1&size={size}"" onclick=""return {pDisable};"">First</a>
            <a href=""?aid{movie.Id}&page={page - 1}&size={size}"" onclick=""return {pDisable};"">Previous</a>
            <span>Page {page} of {pageCount}</span>
            <a href=""?aid{movie.Id}&page={page + 1}&size={size}"" onclick=""return {nDisable};"">Next</a>
            <a href=""?aid{movie.Id}&page={pageCount}&size={size}"" onclick=""return {nDisable};"">Last</a>
        </div>    
